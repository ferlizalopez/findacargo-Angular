angular.module('weeklyDeliveries', ['ngAutocomplete', 'ngFileUpload', 'ui.bootstrap', 'angular.chosen'])
    .controller('weeklyDeliveryController', ['Upload', '$window', '$scope', '$http', '$timeout', function (Upload, $window, $scope, $http, $timeout) {
        $scope.scheduledSetting = user && user.scheduledSetting && user.scheduledSetting.pickupAddress;
        $scope.week = '';
        $scope.error = '';
        $scope.replace = '';
        $scope.fileError = "";
        ...
        $scope.uploadOption = localStorage.getItem('uploadOption');
        ...
        $scope.mappingArray = [
            ...
            {value: 'pickupzip', description: 'Pickup Zip'},
            {value: 'pickupcity', description: 'Pickup City'},
            {value: 'pickupdeadline', description: 'Pickup Deadline'},
            {value: 'deliveryaddress', description: 'Delivery Address'},
            {value: 'deliveryzip', description: 'Delivery Zip'},
            {value: 'deliverycity', description: 'Delivery City'},
            ...
            {value: 'driveremail', description: 'Driver Email'}
        ];

        var tempDeliveryid = 'deliveryid',
            tempPickupAddress = 'pickupaddress',
            tempDeliveryAddress = 'deliveryaddress',
            tempDeliveryDayofWeek = 'deliverydayofweek',
            tempRoutes = 'routes';
        ...
        $scope.upload = function (file, routesWeek) {
            ...
            Upload.upload({
                ...
                if (resp.data.error_code === 0) {
                    ...
                    var allDeliveries = localStorage.setItem(JSON.stringify(resp.data.data.data), 'allDeliveries');
        ...
        $scope.createMapping = function (key, value) {
            ...
            switch(value) {
                case 'deliveryid':
                    ...
                    break;
                case 'pickupaddress':
                    tempPickupAddress = key;
                    break;
                case 'deliveryaddress':
                    tempDeliveryAddress = key;
                    break;
                case 'deliverydayofweek':
                    tempDeliveryDayofWeek = key;
                    break;
                case 'routes':
                    tempRoutes = key;
                    break;
        ...
        $scope.next = function () {
            var allDeliveries = JSON.parse(localStorage.getItem('allDeliveries'));
            _.forEach(allDeliveries, function (delivery) {
                    ...
                    validateDeliveriesData($scope.allDeliveries, function (deliveryError,success) {
                        if(deliveryError){
                            ...
                            $http.post('/api/validateAddress', {
                                    ...
                                    pAddress: tempPickupAddress,
                                    dAddress: tempDeliveryAddress
                                })
                                .then(
                                    function (response) {
            ...
            $http.post('/api/uploadDeliveries', uploadInfo)
                .then(
                    function (response) {
                        ...
                        $window.location.href = '/deliveriesSubmit';
                    },
                    function (err) {
                        console.log(err);
                    }
        ...
        function checkMapping(delivery,callback) {
            var mappingError = '';
            if(delivery) {
                if(!(delivery.hasOwnProperty(tempDeliveryid))){
                    mappingError = {
                        mapError : 1,
                        error:'Delivery Id is mandatory.'
                    }
                }
                if(!(delivery.hasOwnProperty(tempPickupAddress))){
                    mappingError = {
                        mapError : 1,
                        error:'Pickup address is mandatory.'
                    }
                }
                if(!(delivery.hasOwnProperty(tempDeliveryAddress))){
                    mappingError = {
                        mapError : 1,
                        error:'Delivery address is mandatory.'
                    }
                }
                if(!(delivery.hasOwnProperty(tempDeliveryDayofWeek))){
                    mappingError = {
                        mapError : 1,
                        error:'Delivery day of week is mandatory.'
                    }
                }
                if(!(delivery.hasOwnProperty(tempRoutes))){
                    mappingError = {
                        mapError : 1,
                        error:'Routes is mandatory.'
                    }
                }
        ...
        function validateDeliveriesData(validResult,callback) {
            ...
            for (var i = 0; i < validResult.length; i++) {
                if (validResult[i][tempDeliveryid] == "" || validResult[i][tempDeliveryid] == null || validResult[i][tempDeliveryid] == undefined) {
                    error = {
                        error_code: 0,
                ...
                if (validResult[i][tempDeliveryAddress] == "" || validResult[i][tempDeliveryAddress] == null || validResult[i][tempDeliveryAddress] == undefined) {
                    error = {
                        error_code: 2,
                ...
                if (validResult[i][tempPickupAddress] == "" || validResult[i][tempPickupAddress] == null || validResult[i][tempPickupAddress] == undefined) {
                    error = {
                        error_code: 3,
                        fileError: "Pickup Address is mandatory(row" + (i + 1) + ")"
                    };
                    break;
                }
                if (validResult[i][tempDeliveryDayofWeek] == "" || validResult[i][tempDeliveryDayofWeek] == null || validResult[i][tempDeliveryDayofWeek] == undefined) {
                    error = {
                        error_code: 4,
                ...
                if (validResult[i][tempRoutes] == "" || validResult[i][tempRoutes] == null || validResult[i][tempRoutes] == undefined) {
                    error = {
                        error_code: 4,
                        fileError: "Routes is mandatory(row" + (i + 1) + ")"
                    };
                    break;
                }
            }
