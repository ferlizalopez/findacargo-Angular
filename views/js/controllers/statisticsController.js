angular.module('statistics', ['customModule'])
    .controller('statisticsController', function($scope, $http,alert) {
        $scope.chart = {};
        ...
        function arrangeStatsData(statsData, start, end){
            ...
            for(var d = new Date(start.format('MMMM D, YYYY')); d <= new Date(end.format('MMMM D, YYYY')); d.setDate(d.getDate() + 1)){
                var oneDayStats = _.filter(statsData, function(dayStats){
                ...
                if(oneDayStats && oneDayStats.length){
                    ...
                    var onTime = _.filter(oneDayStats[0].data, function(delivery){
                    ...
                    oneDayStats[0].completedDeliveries = _.filter(oneDayStats[0].data, function(delivery){
                        return delivery.carrier && delivery.carrier.status == 7;
                    });
                    oneDayStats[0].nonCompletedDeliveries = _.filter(oneDayStats[0].data, function(delivery){
                        return (!(delivery.carrier) || (delivery.carrier && delivery.carrier.status != 7));
                    });
        ...
        function cb(start, end, range) {
            ...
            if(new Date(start.format('MMMM D, YYYY')) > now){
                ...
                $('#reportrange span').html(start.format('MMMM D, YYYY') + ' - ' + end.format('MMMM D, YYYY'));
                $http.get('/api/statisticsData', {params: {start: start.format('MMMM D, YYYY'), end: end.format('MMMM D, YYYY')}}).then(function (res) {
