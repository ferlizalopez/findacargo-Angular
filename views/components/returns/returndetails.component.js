...
module.component("returndetails", {
    templateUrl: "../components/returns/returndetails.component.html",
    controllerAs: "model",
    controller: ["$http", "$stateParams", "$scope", "$window", "$timeout", "statusService", "socketService", "urlService", "deliveryService", "storageService", "$translate", controller]
});

function controller($http, $stateParams, $scope, $window, $timeout, statusService, socketService, urlService, deliveryService, storageService, $translate) {
    ...
    this.initializeMap = () => {
        let pointA = { lng: parseFloat(this.pickupcoordinates[0]), lat: parseFloat(this.pickupcoordinates[1]) },
            ...
            console.log("poitn ", pointB)
        /*Map fit for two position*/
        /*let bounds = new google.maps.LatLngBounds();
        bounds.extend(new google.maps.LatLngBounds(pointA.lat, pointA.lng));
        bounds.extend(new google.maps.LatLngBounds(pointB.lat, pointB.lng));
